...
class Exception(Base, TimestampMixin):
    ...
    # type + basic descriptors
    type_id: Mapped[int] = mapped_column(ForeignKey("exception_types.id", ondelete="RESTRICT"), index=True)
    title:   Mapped[str] = mapped_column(String(255))
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    severity:    Mapped[Optional[str]] = mapped_column(String(16), nullable=True)  # e.g., LOW/MED/HIGH
    bu_id:       Mapped[Optional[str]] = mapped_column(String(64), nullable=True)  # business unit tag
